...
void GameBoard::fill(int seed, bool startup)
{
    ...
    // If this is a startup, create the number tile objects of the board.
    // If not, set the values of the current number tiles to zero.
    if (startup)
    {
...
void GameBoard::fill_with_zeros()
{
    for( auto y = 0; y < SIZE; ++y )
    {
      for( auto x = 0; x < SIZE; ++x )
      {
          board_.at(y).at(x)->set_value_to_zero();
      }
    }
}
